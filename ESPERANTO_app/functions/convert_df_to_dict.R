...
#' (tmp_upd_vocabolary) is structured in the reference vocabulary fashion, "reference label", "label synonyms", "reference content", "content synonyms". <br />
#'
...
#' tmp_upd_vocabolary <- rbind(dict_keys, new_entries)
...
convert_df_to_dict <- function (tmp_upd_vocabolary){
  tmp <- tmp_upd_vocabolary %>%
    ...
    dplyr::mutate(compact_lab_synlab = stringr::str_c(na.omit(unique(lab_syn)), collapse="|"))  %>%
    tidyr::unite(process_key, c(label,compact_lab_synlab), sep="_*_", remove=FALSE,na.rm=T) %>%
    tidyr::unite(extended_key, c(process_key,allowed_features), sep="_**_", remove=TRUE,na.rm=T) %>%
    dplyr::group_by(extended_key)%>%
    dplyr::mutate(synfeat2 = stringr::str_c(na.omit(unique(syn_features)), collapse="|"))
